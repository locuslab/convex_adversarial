...
class DualNetBoundsBatch: 
    def __init__(self, net, X, epsilon):
        ...
        nu = []
        nu_hat = self.affine[0](X)
        nu_hat_ = self.affine[0](Variable(torch.eye(self.affine[0].in_features)).type_as(X)).unsqueeze(0)
        
        self.zl = [nu_hat + gamma[0] - epsilon*(nu_hat_).abs().sum(1)]
        self.zu = [nu_hat + gamma[0] + epsilon*(nu_hat_).abs().sum(1)]
        ...
        for i in range(0,self.k-2):
            ...
            self.zl.append(nu_hat + sum(gamma) - epsilon*nu_hat_.abs().sum(1) + 
                           sum([(self.zl[j][self.I[j]] * (-nu[j]).clamp(min=0)).mm(I_collapse[j]).t()
                                for j in range(i+1) if not self.I_empty[j]]))
            self.zu.append(nu_hat + sum(gamma) + epsilon*nu_hat_.abs().sum(1) - 
                           sum([(self.zl[j][self.I[j]] * nu[j].clamp(min=0)).mm(I_collapse[j]).t()
                                for j in range(i+1) if not self.I_empty[j]]))
